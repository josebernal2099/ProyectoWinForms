...
namespace DALGeneral
{
    ...
    {
        private static string tagDeCadenaDeConexion = string.Empty;


        /// <summary>
        /// Establece el la etiqueta que contiene la cadena de conexión en el archivo de configuración
        /// </summary>
        public static string TagDeCadenaDeConexionEnConnectionString
        {
            set { value = tagDeCadenaDeConexion; }
        }
        ...
        public static string ConnectionString
        {
            get
            {
                ...
                try
                {
                    ...
                    cadenaConexion = ObtenerCadenaDeConexion("Connection");
                    /*Verifica si la cadena de conexión esta encriptada, solo se busca una palabra entendible
                     * que es DATA SOURCE, si no existe se infiere que está encriptado, de lo contrario se toma
                     * el valor que trae la cadena.
                     * La desencriptacion se toma por default con la que se tiene en Utilerias de este mismo 
                     * proyecto, si se utiliza un algoritmo diferente se debera de cambiar la sentencia:
                     * Utilerias.Encriptador.Desencriptar por la clase y propiedad de encriptacion utilizada
                    */
                    if (CadenaDeConexionEncriptada(cadenaConexion))
                        cadenaConexion = Utilerias.Encriptador.Desencriptar(cadenaConexion);

                    return cadenaConexion;
                }
                catch (Exception error)
                {
                    throw new Exception("Ocurrio un error al tratar de recuperar la cadena de conexión en el archivo de configuración, Detalle: " + error.Message);
                }
            }
        }
        ...
        /// </summary>
        protected RowUpdatingHandler _rowUpdating;
        /// <summary>
        /// Internal handler used for bubbling up the event to the user
        /// </summary>
        protected RowUpdatedHandler _rowUpdated;
        #endregion
        ...
        /// <remarks>
        /// e.g.:  
        ///  XmlReader r = adoHelper.ExecuteXmlReader(command);
        /// </remarks>
        ...
        protected internal abstract void AddUpdateEventHandlers(IDbDataAdapter dataAdapter, RowUpdatingHandler rowUpdatingHandler, RowUpdatedHandler rowUpdatedHandler);
        ...
        /// <example>
        /// AdoHelper helper = AdoHelper.CreateHelper("Elektra.Services.DataAccess", "Elektra.Services.DataAccess.OleDb");
        /// </example>
        public static AdoHelper CreateHelper(string providerAssembly, string providerType)
        {
            //conertimos a fullname
            if (providerAssembly.ToLower().Trim() == "DALGeneral")
            {
                providerAssembly = Assembly.GetExecutingAssembly().FullName;
            }
        ...
        /// <summary>
        /// Crea la instancia del sqlServer directamente.
        /// </summary>
        /// <returns>Una instancia del helper para acceder a la base de datos.</returns>
        ...
        protected virtual void PrepareCommand(IDbCommand command, IDbConnection connection, IDbTransaction transaction, CommandType commandType, string commandText, IDataParameter[] commandParameters, out bool mustCloseConnection)
        {
            ...
            // Cambiamos el TimeOut default por 60 seg.
            command.CommandTimeout = this.ObtenerTiempoEspera();
        ...
        /// <summary>
        /// Obtener el tiempo de espera para realizar alguna accion en BD
        /// </summary>
        /// <returns>segundos</returns>
        private int ObtenerTiempoEspera()
        {
            int segundos = 60;
            bool existeTiempoDeEspera = false;

            try
            {

                // Verifica si existe el timeOut en la cadena de conexion
                string[] parametros = AdoHelper.ConnectionString.Split(';');
                foreach (string parametro in parametros)
                {
                    if (parametro.ToLower().Contains("timeout"))
                    {
                        string[] valores = parametro.Split('=');
                        segundos = Convert.ToInt32(valores[1].Trim());
                        existeTiempoDeEspera = true;
                        break;
                    }
                }

                // Si no existe el tiempo de espera en la cadena de conexion, se busca en el tag del appSettings 
                if (!existeTiempoDeEspera)
                {
                    if (ConfigurationManager.AppSettings != null &&
                        ConfigurationManager.AppSettings["TimeOut"] != null)
                        segundos = Convert.ToInt32(ConfigurationManager.AppSettings["TimeOut"].ToString());
                }
            }
            catch
            {
        ...
        /// <summary>
        /// Valida si la cadena de conexión esta encriptada o no
        /// </summary>
        /// <param name="cadena">Cadena de conexión a validar</param>
        /// <returns></returns>
        private static bool CadenaDeConexionEncriptada(string cadena)
        {
            if (cadena.ToUpper().Contains("DATA SOURCE"))
                return false;
            else
                return true;
        }
        ...
        /// <summary>
        /// Metodo que obtiene los datos del tag de ConnectionStrings del archivo de configuracion
        /// esto valida primero que el tagDeCadena de  conexion exista, y asi asegurar que se tome
        /// el archivo de configuracion del cliente y no el del Machine.config
        /// </summary>
        /// <param name="tagParaObtener">Nombre del tag de cadena de conexion que se desea obtener 
        /// el contenido </param>
        /// <returns></returns>
        private static string ObtenerCadenaDeConexion(string tagParaObtener)
        {
            string respuesta = string.Empty;
            bool esConfigLocal = false;

            foreach (ConnectionStringSettings tag in ConfigurationManager.ConnectionStrings)
            {
                if (tag.Name.ToLower() == tagParaObtener.ToLower())
                {
                    esConfigLocal = true;
                    break;
                }
            ...
            if (esConfigLocal)
                respuesta = string.IsNullOrEmpty(tagDeCadenaDeConexion) ? ConfigurationManager.ConnectionStrings[tagParaObtener].ToString() : ConfigurationManager.ConnectionStrings[tagDeCadenaDeConexion].ToString();
            else
            {
                //con esta instruccion se obtiene la ruta de la dll que utiliza el archivo de configuracion
                //Assembly.GetExecutingAssembly().CodeBase;
                UriBuilder uri = new UriBuilder(Assembly.GetExecutingAssembly().CodeBase);
                string directorioAbuscarConfig = System.IO.Path.GetDirectoryName(Uri.UnescapeDataString(uri.Path));
                string nombreDeArchivoConfig = string.Empty;

                foreach (var archivo in new System.IO.DirectoryInfo(directorioAbuscarConfig).GetFiles())
                {
                    if (archivo.Extension.ToLower().Equals(".config"))
                    {
                        nombreDeArchivoConfig = archivo.Name;
                        break;
                    }
                }
                ExeConfigurationFileMap mapeoDeArchivoConfig = new ExeConfigurationFileMap();
                mapeoDeArchivoConfig.ExeConfigFilename = string.Format(@"{0}\{1}", directorioAbuscarConfig, nombreDeArchivoConfig);
                Configuration config = ConfigurationManager.OpenMappedExeConfiguration(mapeoDeArchivoConfig,
                                                                                       ConfigurationUserLevel.None);

                respuesta = config.ConnectionStrings.ConnectionStrings[tagParaObtener].ToString();
            }
        ...
        /// <summary>
        /// Execute a IDbCommand (that returns a resultset) against the database specified in 
        ...
        /// <summary>
        /// Execute a IDbCommand (that returns no resultset) against the database
        /// </summary>
        ...
        /// <summary>
        /// Execute a IDbCommand (that returns a 1x1 resultset) against the database specified in 
        /// the connection string. 
        /// </summary>
        /// <param name="command">The IDbCommand to execute</param>
        ...
        /// <remarks>
        /// e.g.:  
        ///  XmlReader r = ExecuteXmlReader(conn, CommandType.StoredProcedure, "GetOrders", GetParameter("@prodid", 24));
        /// </remarks>
        /// <param name="connection">A valid IDbConnection</param>
        /// <param name="commandType">The CommandType (stored procedure, text, etc.)</param>
        /// <param name="commandText">The stored procedure name or SQL command using "FOR XML AUTO"</param>
        ...
        /// <remarks>
        /// e.g.:  
        ///  XmlReader r = ExecuteXmlReader(trans, CommandType.StoredProcedure, "GetOrders", GetParameter("@prodid", 24));
        /// </remarks>
        /// <param name="transaction">A valid IDbTransaction</param>
        /// <param name="commandType">The CommandType (stored procedure, text, etc.)</param>
        /// <param name="commandText">The stored procedure name or SQL command using "FOR XML AUTO"</param>
        ...
        protected internal void RowUpdating(object obj, System.Data.Common.RowUpdatingEventArgs e)
        {
        ...
        /// <param name="obj">The object that generated the event</param>
        /// <param name="e">The System.Data.Common.RowUpdatingEventArgs</param>
        protected internal void RowUpdated(object obj, System.Data.Common.RowUpdatedEventArgs e)
        {
        ...
        /// <remarks> 
        /// e.g.:  
        /// SqlRowUpdatingEventHandler rowUpdatingHandler = new SqlRowUpdatingEventHandler( OnRowUpdating ); 
        /// SqlRowUpdatedEventHandler rowUpdatedHandler = new SqlRowUpdatedEventHandler( OnRowUpdated ); 
        /// adoHelper.UpdateDataSet(sqlInsertCommand, sqlDeleteCommand, sqlUpdateCommand, dataSet, "Order", rowUpdatingHandler, rowUpdatedHandler); 
        /// </remarks> 
        /// <param name="insertCommand">A valid transact-SQL statement or stored procedure to insert new records into the data source</param> 
        /// <param name="deleteCommand">A valid transact-SQL statement or stored procedure to delete records from the data source</param> 
        /// <param name="updateCommand">A valid transact-SQL statement or stored procedure used to update records in the data source</param> 
        ...
        /// <summary>
        /// Execute a stored procedure via a IDbCommand (that returns no resultset) 
